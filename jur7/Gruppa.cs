...
namespace jur7
{
    public partial class Gruppa : Form
    {
        ...
        public void run_main()
        {
            try
            {
                var query = "select * from gruppa_jur7";
        ...
        private void Gruppa_Load(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Gruppa_Load " + ex.Message, "Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        private void gruppa__dataGridView_DataError(object sender, DataGridViewDataErrorEventArgs e)
        {
            if (e.Exception.Message == "DataGridViewComboBoxCell value is not valid.")
            {
                object value = gruppa__dataGridView.Rows[e.RowIndex].Cells[e.ColumnIndex].Value;
                if (!((DataGridViewComboBoxColumn)gruppa__dataGridView.Columns[e.ColumnIndex]).Items.Contains(value))
                {
                    ((DataGridViewComboBoxColumn)gruppa__dataGridView.Columns[e.ColumnIndex]).Items.Add(value);
                    e.ThrowException = false;
                }
            }
        }

        private void gruppa__dataGridView_UserDeletingRow(object sender, DataGridViewRowCancelEventArgs e)
        {
            try
            {
                ...
                {
                    foreach (DataGridViewRow row in gruppa__dataGridView.SelectedRows)
                    {
            ...
            {
                MessageBox.Show("gruppa__dataGridView_UserDeletingRow " + ex.Message);
            }
        }
    }
}
