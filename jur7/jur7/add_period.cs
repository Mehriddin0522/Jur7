...
namespace jur7
{
    public partial class add_period : Form
    {
        ...
        {
            var query = " SELECT * FROM pereosenka_data ";
        ...
        private void podrazdelenie_dataGridView_CellValueChanged(object sender, DataGridViewCellEventArgs e)
        {
                ...
                if (dgvRow.Cells[0].Value == null)
            {
                Console.WriteLine("insert");
                    var ins = "insert into pereosenka_data (number,date_start,date_finish) values" +
                ...
                sql1.myReader = sql1.return_MySqlCommand("select max(id) as id from pereosenka_data").ExecuteReader();
                while (sql1.myReader.Read())
                {
            ...
            else
            {
                Console.WriteLine("update " + dgvRow.Cells[0].Value);
                    ...
                    var update = "update pereosenka_data set " +
